...
class WebhookController extends Controller
{
        ...
        switch ($dataType) {
            case 'transaksi':
                ...
                if ($transactions->isEmpty()) {
                    return response()->json(['reply' => 'Tidak ada data transaksi untuk diekspor.']);
                }
            ...
            case 'investasi':
                ...
                if ($portfolio->isEmpty()) {
                    return response()->json(['reply' => 'Tidak ada data investasi untuk diekspor.']);
                }
            ...
            case 'hutang':
                ...
                if ($debts->isEmpty()) {
                    return response()->json(['reply' => 'Tidak ada data hutang/piutang untuk diekspor.']);
                }
            ...
            case 'tabungan':
                ...
                if ($savings->isEmpty()) {
                    return response()->json(['reply' => 'Tidak ada data tabungan untuk diekspor.']);
                }
    ...
    private function handleUseEmergencyFund($phone, $data)
    {
        ...
        Transaction::create([
            ...
            'type' => 'pemasukan',
            'amount' => $data['amount'],
            'description' => "Pakai Dana Darurat: {$data['description']}",
            'category' => 'dana darurat',
        ]);
...
PROMPT;
    ...
    private function handleShowReport($phone, $data)
    {
        ...
        switch ($reportType) {
            ...
            case 'tabungan':
                ...
                $emergencyFundReport = $this->generateEmergencyFundReport($phone, false); 
    ...
    private function generateDebtReport($phone)
    {
        ...
        if (!empty($piutangList)) {
            $reply .= "✅ *Uang Anda di Luar (Piutang):*\n" . $piutangList;
        }
        if (!empty($hutangList)) {
    ...
    private function generateCashFlowReport($phone, $period)
    {
        ...
        if ($transactions->isEmpty()) {
            return "💸 *Laporan Arus Kas {$periodName}*\n\nTidak ada transaksi.";
        }
